...
export default function ChooseTokenDialog() {
    const { tokenData, setTokenInPrice, tokenInPrice, tokenInEntity, setTokenInEntity, ratesData, setRatesData } = useRoot()
    ...
    useEffect(() => {
        if (dialog) {
            setOldTokenInEntity(tokenInEntity)
            setOldActiveChainId(activeChainId)
        } else {
            if (tokenInEntity.address == oldTokenInEntity?.address) {
    ...
    useEffect(() => {
        ...
        if (tokenData) {
            ...
            if (!tokenInEntity?.name) setTokenInEntity(tokenData[0])
        }
    }, [tokenInEntity, tokenData, search, activeChainId])
    return (
        <Dialog.Root open={dialog} onOpenChange={() => { setDialog(!dialog) }}>
            <Dialog.Trigger asChild>
                <button className="flex items-center gap-2 h-10 text-[24px] font-normal font-['Onest'] px-2 rounded-lg border border-transparent hover:border-white/12 text-white">
